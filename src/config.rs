...
impl Config {
    ...
    pub fn save_to_path(&self, path: &Path) -> Result<()> {
        ...
        // The token lives in this 0600 file (dir 0700) — deliberately no OS
        // keychain: macOS binds keychain ACLs to the binary's code signature, so
        // every self-update re-prompted on each command. File-based storage is
        // what gh/aws/flyctl (and orin) ship.
        let to_persist = self.clone();
